...
def download(url, directory="trained_models"):
...
def SentenceTransformerViz(model, source, target, threshold):
    ...
    if score < threshold:
        printmd("Score: " + make_color((str("%.4f " % score)), 'red'))
    else:
        printmd("Score: " + make_color((str("%.4f " % score)), 'green'))
